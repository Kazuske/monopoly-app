...
export async function updateTeamCash(p_amount, p_team_id) {
  const { data, error } = await supabase.rpc('update_team_cash', { p_amount, p_team_id });
  if (error) return console.error('❌ Error updating team cash:', error);
  printJSONTable('🔁 Team Cash Updated', data);
  return data;
}
